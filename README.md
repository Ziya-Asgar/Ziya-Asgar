# My Learnings and Portfolio of Projects

## Intro

I have educational background in Finance and Economics, and interests in various areas. I use this space to document my journey in Data and Coding, sharing both learnings and projects. I have grouped my repositories for easy reference under different categories. You can learn using my notes, explore and practice on my projects. Here are more details about my repositories:

- [My Learnings and Portfolio of Projects](#my-learnings-and-portfolio-of-projects)
  - [Intro](#intro)
  - [Learning Resources](#learning-resources)
  - [Data Analysis and Visualization Resources](#data-analysis-and-visualization-resources)
    - [Projects](#projects)
    - [Learning Repositories for Data Analysis and Visualization](#learning-repositories-for-data-analysis-and-visualization)
  - [Database-related Resources](#database-related-resources)
    - [Projects](#projects-1)
    - [Learning Repositories for Databases](#learning-repositories-for-databases)
  - [SQL Resources](#sql-resources)
    - [Projects](#projects-2)
    - [Learning Repositories for SQL](#learning-repositories-for-sql)
  - [Power BI Resources](#power-bi-resources)
  - [Python Resources](#python-resources)
    - [Projects](#projects-3)
    - [Learning Repositories for Python](#learning-repositories-for-python)
  - [Web Development Resources](#web-development-resources)

---

## Learning Resources

These repositories contain the notes and cheatsheets that I prepared while learning and practicing various tools and topics. These repositories provide extensive notes and cheatsheets to help you understand various tools and topics. They serve as a valuable reference for quickly recalling concepts and implementing technology with an example. I update and rewrite these notes, and add more repositories as I learn new things. Feel free to check these out:

- [PostgreSQL notes](https://github.com/Ziya-Asgar/PostgreSQL)
- [MongoDB notes](https://github.com/Ziya-Asgar/MongoDB)
- [Python notes](https://github.com/Ziya-Asgar/Python)
- [Python Pandas notes](https://github.com/Ziya-Asgar/Python_Pandas)
- [Python Numpy notes](https://github.com/Ziya-Asgar/Python_Numpy)
- [Python Matplotlib notes](https://github.com/Ziya-Asgar/Python_Matplotlib)
- [Python Seaborn notes](https://github.com/Ziya-Asgar/Python_Seaborn)
- [GIT notes](https://github.com/Ziya-Asgar/GIT)
- [JavaScript notes](https://github.com/Ziya-Asgar/JavaScript)
- [NodeJS and ExpressJS notes](https://github.com/Ziya-Asgar/NodeJS)
- [ReactJS notes](https://github.com/Ziya-Asgar/ReactJS)

---

## Data Analysis and Visualization Resources

These repositories contain the notes and projects related to **data analysis and visualization**. You can learn new tools and practice with the projects below to improve your data analysis and visualization skills:

### Projects

- [Controlling Excel Files with Python](https://github.com/Ziya-Asgar/11_python_automate_excel)
- [Data Cleaning Project with Python Pandas](https://github.com/Ziya-Asgar/10_pandas_data_cleaning)
- [Several Data Analysis and Practice Projects using Python Pandas](https://github.com/Ziya-Asgar/09_pandas_project_and_practice_sessions)
- [Sales Dashboard Project with SQL and Power BI](https://github.com/Ziya-Asgar/22_pizza_sales_SQL_PBI)
- [SQL Data Analysis Project](https://github.com/Ziya-Asgar/15_SQL_DA)
- [Power BI Project with a PostgreSQL DB](https://github.com/Ziya-Asgar/14_SQL_PBI)
- [SQL Exploratory Data Analysis project](https://github.com/Ziya-Asgar/13_SQL_EDA)

### Learning Repositories for Data Analysis and Visualization

- [PostgreSQL notes](https://github.com/Ziya-Asgar/PostgreSQL)
- [MongoDB notes](https://github.com/Ziya-Asgar/MongoDB)
- [Python notes](https://github.com/Ziya-Asgar/Python)
- [Python Pandas notes](https://github.com/Ziya-Asgar/Python_Pandas)
- [Python Numpy notes](https://github.com/Ziya-Asgar/Python_Numpy)
- [Python Matplotlib notes](https://github.com/Ziya-Asgar/Python_Matplotlib)
- [Python Seaborn notes](https://github.com/Ziya-Asgar/Python_Seaborn)

---

## Database-related Resources

These repositories contain the notes and projects related to **databases**. This section provides information-rich resources for both relational (PostgreSQL) and non-relational (MongoDB) database management systems. The projects focus on the relational databases. You can learn new tools and practice with the projects below to improve your database-related skills:

### Projects

- [Sales Dashboard Project with SQL and Power BI](https://github.com/Ziya-Asgar/22_pizza_sales_SQL_PBI)
- [SQL Data Analysis Project](https://github.com/Ziya-Asgar/15_SQL_DA)
- [Power BI Project with a PostgreSQL DB](https://github.com/Ziya-Asgar/14_SQL_PBI)
- [SQL Exploratory Data Analysis project](https://github.com/Ziya-Asgar/13_SQL_EDA)

### Learning Repositories for Databases

- [PostgreSQL notes](https://github.com/Ziya-Asgar/PostgreSQL)
- [MongoDB notes](https://github.com/Ziya-Asgar/MongoDB)

---

## SQL Resources

These repositories contain the notes and projects related to the **Structured Query Language (SQL)**. The learning resources here cover extensive information about the PostgreSQL database management system. The projects involve applying various SQL commands and statements. You can learn and practice with the projects below to improve your SQL skills:

### Projects

- [Sales Dashboard Project with SQL and Power BI](https://github.com/Ziya-Asgar/22_pizza_sales_SQL_PBI)
- [SQL Data Analysis Project](https://github.com/Ziya-Asgar/15_SQL_DA)
- [Power BI Project with a PostgreSQL DB](https://github.com/Ziya-Asgar/14_SQL_PBI)
- [SQL Exploratory Data Analysis project](https://github.com/Ziya-Asgar/13_SQL_EDA)

### Learning Repositories for SQL

- [PostgreSQL notes](https://github.com/Ziya-Asgar/PostgreSQL)

---

## Power BI Resources

These repositories involve using **Power BI** to visualize, transform data with **Power Query**, and calculate various data dimensions using **Data Analysis Expressions (DAX)**. You can practice with the projects below to improve your Power BI skills:

- [Sales Dashboard Project with SQL and Power BI](https://github.com/Ziya-Asgar/22_pizza_sales_SQL_PBI)
- [Power BI Project with a PostgreSQL DB](https://github.com/Ziya-Asgar/14_SQL_PBI)

---

## Python Resources

These repositories contain the notes and projects related to **Python**, and its various libraries such as **Pandas**, **Numpy**, **Matplotlib**, and **Seaborn**. The learning resources here cover extensive information about Python and its various libraries together with providing lots of examples. You can learn with notes and practice with the projects below to improve your Python skills to an excellent level:

### Projects

- [Controlling Excel Files with Python](https://github.com/Ziya-Asgar/11_python_automate_excel)
- [Data Cleaning Project with Python Pandas](https://github.com/Ziya-Asgar/10_pandas_data_cleaning)
- [Several Data Analysis and Practice Projects using Python Pandas](https://github.com/Ziya-Asgar/09_pandas_project_and_practice_sessions)

### Learning Repositories for Python

- [Python notes](https://github.com/Ziya-Asgar/Python)
- [Python Pandas notes](https://github.com/Ziya-Asgar/Python_Pandas)
- [Python Numpy notes](https://github.com/Ziya-Asgar/Python_Numpy)
- [Python Matplotlib notes](https://github.com/Ziya-Asgar/Python_Matplotlib)
- [Python Seaborn notes](https://github.com/Ziya-Asgar/Python_Seaborn)

---

## Web Development Resources

This section contains notes related to **web development**, with a focus on **JavaScript**. The learning resources here cover details about frontend, backend, and databases. These repositories are extensive and involve lots of examples. You can learn and practice with these notes to improve your web development skills to a good level:

- [JavaScript notes](https://github.com/Ziya-Asgar/JavaScript)
- [ReactJS notes](https://github.com/Ziya-Asgar/ReactJS)
- [NodeJS and ExpressJS notes](https://github.com/Ziya-Asgar/NodeJS)
- [GIT notes](https://github.com/Ziya-Asgar/GIT)
- [MongoDB notes](https://github.com/Ziya-Asgar/MongoDB)
- [PostgreSQL notes](https://github.com/Ziya-Asgar/PostgreSQL)

---
